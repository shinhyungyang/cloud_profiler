...
#For old versions, use archive.apache.org
...
MEASUREMENT_GRAPH="${HOME}/git/cloud_profiler/src/cp/tsc/rtt_scheduler/meas_graph.csv"
if [[ ! -e "$MEASUREMENT_GRAPH" ]]
then
  echo "WARNING: Measurement graph does not exist"
fi
...
run() {
  ...
  ##$GIT clean -fd
	...
	echo 'redis.host: "127.0.0.1"' >> $CONF_FILE
    ...
    $MVN clean install -Dspark.version="$SPARK_VERSION" -Dkafka.version="$KAFKA_VERSION" -Dflink.version="$FLINK_VERSION" -Dstorm.version="$STORM_VERSION" -Dscala.binary.version="$SCALA_BIN_VERSION" -Dscala.version="$SCALA_BIN_VERSION.$SCALA_SUB_VERSION" -s ./settings.xml
    ...
    fetch_untar_file "$ZK_FILE" "$APACHE_MIRROR/zookeeper/$ZK_DIR/$ZK_FILE"
...
#   fetch_untar_file "$SPARK_FILE" "$APACHE_MIRROR/spark/spark-$SPARK_VERSION/$SPARK_FILE"
  ...
  then
    start_if_needed org.apache.storm.daemon.nimbus "Storm Nimbus" 3 "$STORM_DIR/bin/storm" nimbus
  ##start_if_needed org.apache.storm.daemon.supervisor "Storm Supervisor" 3 "$STORM_DIR/bin/storm" supervisor
    start_if_needed org.apache.storm.ui.core "Storm UI" 3 "$STORM_DIR/bin/storm" ui
    start_if_needed org.apache.storm.daemon.logviewer "Storm LogViewer" 3 "$STORM_DIR/bin/storm" logviewer
    sleep 10
  elif [ "STOP_STORM" = "$OPERATION" ];
  then
    stop_if_needed org.apache.storm.daemon.nimbus "Storm Nimbus"
  ##stop_if_needed org.apache.storm.daemon.supervisor "Storm Supervisor"
    stop_if_needed org.apache.storm.ui.core "Storm UI"
    stop_if_needed org.apache.storm.daemon.logviewer "Storm LogViewer"
  ...
  then
    start_if_needed org.apache.spark.deploy.master.Master SparkMaster 5 $SPARK_DIR/sbin/start-master.sh -h 127.0.0.1 -p 7077
  ##start_if_needed org.apache.spark.deploy.worker.Worker SparkSlave 5 $SPARK_DIR/sbin/start-slave.sh spark://127.0.0.1:7077
  ...
  then
    "$STORM_DIR/bin/storm" jar ./storm-benchmarks/target/storm-benchmarks-0.1.0.jar storm.benchmark.AdvertisingTopology test-topo -conf $CONF_FILE -measurementgraph "$(grep -o '^[^#]*' ${MEASUREMENT_GRAPH} | sed ':a;N;$!ba;s/\n/;/g')"
    sleep 15
  ...
  then
    "$SPARK_DIR/bin/spark-submit" --master spark://127.0.0.1:7077 --class spark.benchmark.KafkaRedisAdvertisingStream ./spark-benchmarks/target/spark-benchmarks-0.1.0.jar "$CONF_FILE" &
    sleep 5
